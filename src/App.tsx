...
function Login({ onLogin }: { onLogin: (user: User) => void }) {
  ...
  return <main className="login-page"><section className="login-card"><img className="platform-logo-login" src="/icons/stc-icon.svg?v=3" alt="Reconciliação STC" /><p className="eyebrow">PLATAFORMA DE RECONCILIAÇÃO STC</p><h1>Bem-vindo, Diogo</h1><p>Acesso reservado ao proprietário da plataforma Keve.</p><form onSubmit={submit}><label>Utilizador<input value="dabranches" disabled /></label><label>PIN<input value={pin} onChange={(e) => setPin(e.target.value.replace(/\D/g, "").slice(0, 12))} type="password" inputMode="numeric" autoComplete="current-password" minLength={6} required autoFocus /></label>{error && <p className="login-error" role="alert">{error}</p>}<button className="primary" disabled={busy || pin.length < 6}>{busy ? "A validar…" : "Entrar"}</button></form><small>O PIN é verificado no servidor e nunca fica guardado neste dispositivo.</small></section></main>;
}
...
function Workspace({ user, onLogout }: { user: User; onLogout: () => void }) {
  ...
  return <div className="app-shell"><aside><div className="brand"><span className="keve-logo keve-logo-sidebar"><img src="/institutions/keve/keve-logo-green.png" alt="Keve" /></span></div><nav aria-label="Menu principal">{views.map((item) => <button key={item.id} className={view === item.id ? "active" : ""} onClick={() => setView(item.id)}><span>{item.icon}</span>{item.label}{item.id === "pendencias" && <em>{position.closing_pending_count}</em>}</button>)}</nav><button className="sidebar-logout" onClick={onLogout}>Terminar sessão</button><div className="side-foot"><strong>{user.display_name ?? user.displayName}</strong><small>Proprietário · @{user.username}</small></div></aside><main><header><div><strong>{heading}</strong><small className="scope-label">Banco Keve · Reconciliação STC</small></div><span className="period">Posição: <b>{position.position_date}</b></span></header><div className="content">{body}</div></main></div>;
}
